...
// 2. Add state for crown-athletes
c = c.replace(
...
// 3. Add Role Admin button in table - add a column header 
c = c.replace(
...
// 4. Add Role column cell before the action buttons cell
c = c.replace(
  `<TableCell>
...
// Find the closing of the Card and add tabs content close + athletes tab
c = c.replace(
  '      </div>\n    </main>\n  );\n}',
  `      </Card>
          </TabsContent>
            ...
            <Card className="border-cyan-500/20 bg-cyan-950/10 backdrop-blur-md shadow-xl text-slate-100">
                ...
                <div className="flex items-center justify-between">
                  <div>
                    ...
                    <CardDescription className="text-cyan-200/60 mt-1">
                      Nama di sini otomatis muncul di formulir izin latihan (Kalender Jadwal).
                    </CardDescription>
                  </div>
              ...
              <CardContent className="pt-4 space-y-4">
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-3">
                  ...
                  {crownAthletes.length === 0 && (
                    <p className="text-sm text-slate-500 col-span-full">Belum ada atlit. Tambahkan nama di atas.</p>
                  )}
                </div>
              </CardContent>
            </Card>
...
// Remove the duplicate closing </Card> that was originally there
// The original file ends with </Card> </div> </main> which we replaced
// but we need to remove the old </Card> that's now dangling
// Actually let's check - the original closing pattern was:
// </Table></div></CardContent></Card></div></main>
// We replaced </div></main> with our new code, but </Card> was already there
// So we might have an extra </Card>. Let me check.
// The replace above handles `</div>\n    </main>\n  );\n}` which is AFTER </Card>
// So we need to also capture the </Card> before it

fs.writeFileSync(file, c, 'utf8');
console.log('✅ Athletes page patched with Tabs + Admin Role + Dynamic Athletes');
